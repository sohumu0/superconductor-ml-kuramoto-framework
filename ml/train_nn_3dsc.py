#!/usr/bin/env python3
"""
Neural Network (MLP) regression for predicting superconductor Tc from the 3DSC_MP dataset.
Uses MAGPIE + structural features with GroupShuffleSplit cross-validation.
Architecture from the 3DSC project's hyperparameters: 5 layers (100->30) + classifier.
...
def net_pattern(n_layers, base_size, end_size):
    if n_layers != 1:
        ...
        factor = 1
    return tuple(int(round(base_size * factor ** n)) for n in range(n_layers))

HIDDEN_LAYERS = net_pattern(5, 100, 30) + (30,)
...
def main():
    ...
    print(f"NN architecture: {HIDDEN_LAYERS}")
    ...
    print("\n" + "="*70)
    print("Neural Network Results (mean +/- std over {} repetitions)".format(N_REPS))
    print("="*70)
